...
const STORAGE_KEY = 'keytrainer-lessons'
...
function loadRecords(): Record<number, LessonRecord> {
  try {
...
function saveRecords(records: Record<number, LessonRecord>): void {
  try {
...
export default function Lessons() {
  ...
  const [records, setRecords] = useState<Record<number, LessonRecord>>(loadRecords)
  ...
  useEffect(() => {
    ...
    const updated = { ...records, [currentLevel]: newRecord }
    setRecords(updated)
    saveRecords(updated)
  }, [isFinished])
  ...
  const handleStart = useCallback((level: number) => {
    ...
    if (lesson) start(lesson.generate())
  }, [start])
